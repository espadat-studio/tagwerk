...
Exit 2 is shared with argparse, which spends it on a usage error, so `tagwerk doctor --bogus` alarms a prompt exactly as a dark sensor does.
...
A machine that deliberately runs no agents reports `beat` dark for good; [Troubleshooting](/troubleshooting/) covers that and the other known ceilings.
