...
- A terminal window holding more than one login shell resolves no cwd rather than picking one, so a split or tabbed terminal that is not kitty books nothing to a repo (ADR-0016). kitty is exact in that case, because its socket names the focused pane.
